...
char worker_died = 0; /* set to 1 in signal handler if worker died */
		...
		if (worker_died) {
			...
			for (int i = 0; i < conf->workers; i++) {
				if (wpids[i] <= 0) {
					...
					if (wpids[i] < 0) {
						ec = -1;
						_LOG_INFO(root_logger, "%s", "worker restart faled");
						if (workers <= 0)
							break;
					} else {
						_LOG_INFO(root_logger, "worker restarted with %d", wpids[i]);
						workers++;
					}
				}
			}
	...
	while ((pid = waitpid((pid_t)(-1), &wstatus, WNOHANG)) > 0) {
		if (pid > 0) {
			...
			if (WIFEXITED(wstatus)) {
				_LOG_ERROR(root_logger, "worker %d exited with status %d", pid,
				           WEXITSTATUS(wstatus));
			} else if (WIFSIGNALED(wstatus)) {
				_LOG_ERROR(root_logger, "worker %d killed with signal '%s'", pid,
